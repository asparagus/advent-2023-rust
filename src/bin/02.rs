...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    let ids_and_games = non_empty_lines.map(
        |line|
        {
    ...
    let possible_ids_and_games = ids_and_games.filter(
        |(_id, game)|
        cube_regex.captures_iter(game).all(
            |cap|
            validate(
...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    let game_powers = ids_and_games.map(
        |(_id, game)|
        red_regex.captures_iter(game).map(|caps| caps.name("number").unwrap().as_str().parse::<u32>().unwrap()).max().unwrap_or(0)
        *
        blue_regex.captures_iter(game).map(|caps| caps.name("number").unwrap().as_str().parse::<u32>().unwrap()).max().unwrap_or(0)
        *
        green_regex.captures_iter(game).map(|caps| caps.name("number").unwrap().as_str().parse::<u32>().unwrap()).max().unwrap_or(0)
    );
